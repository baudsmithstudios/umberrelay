...
func (l *Listener) handleQuery(w mdns.ResponseWriter, r *mdns.Msg) {
	...
	if resp == nil {
		log.Printf("all upstreams failed for query type=%s", formatQuestionForLog(r))
		m := new(mdns.Msg)
...
func formatQuestionForLog(r *mdns.Msg) string {
	if len(r.Question) == 0 {
		return "<empty>"
	}
	q := r.Question[0]
	if queryType, ok := mdns.TypeToString[q.Qtype]; ok && queryType != "" {
		return queryType
	}
	return fmt.Sprintf("TYPE%d", q.Qtype)
}
