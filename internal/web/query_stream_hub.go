...
func (h *queryStreamHub) Subscribe() (<-chan store.Query, func()) {
	...
	// Wake immediately so new subscribers catch up on any pending backlog.
	h.NotifyNewQueries()
...
func (h *queryStreamHub) pollOnce() {
	...
	for batch := 0; batch < maxPollBatchesPerWake; batch++ {
