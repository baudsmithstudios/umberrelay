...
Scrye's MVP is complete. The next phase should focus on closing the biggest trust and visibility gaps first, then strengthening the privacy story, then expanding scope only where the added complexity is justified.
...
Priorities may change as Scrye matures. This roadmap reflects current product direction, not guaranteed release commitments.

## Priority 1: Close The Biggest Product Gaps

1. **DoH/DoT detection**
   Identify devices bypassing local DNS. If a device uses encrypted DNS, Scrye is blind to it, and users need to know that immediately.

2. **New behavior alerting**
   Alert when a device contacts a never-before-seen domain. This is one of the highest-signal changes Scrye can surface with the data it already has.
...
7. **CLI reports and export**
   Add `scrye report` and `scrye export` commands for summaries, automation, and scripting.
...
15. **TLS fingerprinting**
    Add JA3/JA4-style enrichment once capture infrastructure exists. Useful, but not necessary to prove Scrye's core value.
...
18. **Interactive setup wizard**
    Add `scrye init` for guided first-run setup.
